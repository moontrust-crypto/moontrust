...
contract MoonTrustToken is BEP20 {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        uint256 baseBalance = ERC20.balanceOf(account);
    ...
    function getPoolShare(address account) private view returns (uint256) {
        ...
        uint256 accuracy = 1 ether;
        uint256 sharePercent = (baseBalance * accuracy) / totalSupply();
        uint256 shareActual = (poolBalance * sharePercent) / accuracy;
        return shareActual;
    }
        ...
        returns (uint256)
    {
        ...
        uint256 accuracy = 1 ether;
        uint256 sharePercent = (transferAmount * accuracy) / senderBalance;
        uint256 shareActual = (senderPoolShare * sharePercent) / accuracy;
        return shareActual;
    }
    ...
    function _transfer(
        ...
        if (isSenderWhitelisted(sender) || isReceiverWhitelisted(recipient)) {
            ...
            ERC20._transfer(sender, address(this), splitTax);
